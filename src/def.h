/*
    ...
    This file is the general header file for all parts of the uemacs display
    editor. It contains all of the general definitions and macros. It also
    contains some conditional compilation flags. All of the per-system and
    per-terminal definitions are in special header files. The most common reason
    to edit this file would be to zap the definition of CVMVAS or BACKUP.
*/
...
#define	NFILEN	250			/* Length, file name.		*/
...
#define GOOD    0           /* main() normal return     */
...
/*
    Keys are represented inside using an 11 bit keyboard code. The
    transformation between the keys on the keyboard and 11 bit code is done by
    terminal specific code in the "kbd.c" file. The actual character is stored
    in 8 bits (DEC multinationals work); there is also a control flag KCTRL, a
    meta flag KMETA, and a control-X flag KCTLX. ASCII control characters are
    always represented using the KCTRL form. Although the C0 control set is
    free, it is reserved for C0 controls because it makes the communication
    between "getkey" and "getkbd" easier. The funny keys get mapped into the C1
    control area.
*/
...
/*
    The symbol table links editing functions to names. Entries in the key map
    point at the symbol table entry. A reference count is kept, but it is
    probably next to useless right now. The old type code, which was not being
    used and probably not right anyway, is all gone.
*/
...
/*
    ...
    has happened to the buffer mapped by the window. Setting "WFHARD" is always
    a safe thing to do, but it may do more work than is necessary. Always try to
    set the simplest action that achieves the required update. Because commands
    set bits in the "w_flag", update will see all change flags, and do the most
    general one.
 */
...
/*
    ...
    specification of a region around a little bit easier. There have been some
    complaints that the short in this structure is wrong; that a long would be
    more appropriate. I'll awat more comments from the folks with the little
...
/*
    The rationale behind these macros is that you could (with some editing, like
    changing the type of a line link from a "LINE *" to a "REFLINE", and fixing
    the commands like file reading that break the rules) change the actual
    storage representation of lines to use something fancy on machines with
