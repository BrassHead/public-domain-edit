/*
    uemacs - Terminal independent keyboard handling
...
/*
    Read in a key, doing the terminal independent prefix handling. The terminal
    specific "getkbd" routine gets the first swing, and may return one of the
    special codes used by the special keys on the keyboard. The "getkbd" routine
    returns the C0 controls as received; this routine moves them to the right
    spot in 11 bit code.
*/
int
getkey()
...
/*
    Transform a key code into a name, using a table for the special keys and
    combination of some hard code and some general processing for the rest. None
    of this code is terminal specific any more. This makes adding keys easier.
*/
void
