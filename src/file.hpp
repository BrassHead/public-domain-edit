...
/*
	Read a file into the current buffer. This is really easy; all you do it find
...
/*
	...
	If the BACKUP conditional is set, then this routine also does the read end
	of backup processing. The BFBAK flag, if set in a buffer, says that a backup
	should be taken. It is set when a file is read in, but not on a new file
	(you don't need to make a backup copy of nothing). Return a standard status.
	Print a summary (lines read, error message) out as well.
*/
int
readin(char fname[])
{
...
/*
	Save the contents of the current buffer back into its associated file. Do
	...
	read or visit, then a backup copy of the file is made.
*/
int
...
/*
	This function performs the details of file writing. Uses the file management
	routines in the "fileio.c" package. The number of lines written is
	displayed. Sadly, it looks inside a LINE; provide a macro for this. Most of
	the grief is error checking of some sort.
*/
int
writeout(char *fn)
{
