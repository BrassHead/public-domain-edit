/*
	uemacs - Echo line reading and writing
...
/*
	Send a string to the message system. Add a free newline to the end of the
	message string. Return TRUE if it fits, and FALSE if it does not. Perhaps
	the message buffer should know how to get larger, just like the kill buffer?
*/
int
...
/*
	Read messages. The message lines are displayed, one line at a time, in the 
	message line. A special sub-mode is entered, in which the keys have the 
	following meanings:
	...
	Return TRUE if you left this mode in a reasonable way (not ^G), and ABORT 
...
/*
	...
	with full "eprintf" formatting, although the arguments are in a rather
...
ereply(const char *fp, char *buf, int nbuf, ...)
{
    ...
    va_start(args,nbuf);
    int rc;
    rc = eread(fp, buf, nbuf, EFNEW|EFCR, args);
    va_end(args);
	return rc;
}

/*
	This is the general "read input from the echo line" routine. The basic idea
	is that the prompt string "prompt" is written to the echo line, and a one
	line reply is read back into the supplied "buf" (with maximum length "len").
	The "flag" contains EFNEW (a new prompt), an EFAUTO (autocomplete), or EFCR
	(echo the carriage return as CR).
*/
int
...
/*
	The "sp1" and "sp2" point to extended command symbol table entries. The
	"cpos" is a horizontal position in the name. Return the longest block of
	characters that can be autocompleted at this point. Sometimes the two
...
/*
	Special "printf" for the echo line. Each call to "eprintf" starts a new line
	in the echo area, and ends with an erase to end of the echo line. The
...
/*
	Printf style formatting. This is called by both "eprintf" and "ereply", to
	provide formatting services to their clients. The move to the start of the
	echo line, and the erase to the end of the echo line, is done by the caller.
*/
void
