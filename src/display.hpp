/*
    uemacs - Gosling style redisplay.
    ...
    almost nothing about the editing process; the editing functions do, however,
    set some hints to eliminate a lot of the grinding. There is more that can be
    done; the "vtputc" interface is a real pig. Two conditional compilation
    flags; the GOSLING flag enables dynamic programming redisplay, using the
    algorithm published by Jim Gosling in SIGOA. The MEMMAP changes things
    around for memory mapped video. With both off, the terminal is a VT52.
*/

/*
	Define tabstops every TABMASK+1 columns
*/
#define TABMASK 0x03

/*
	You can change these back to the types implied by the name if you get tight
	for space. If you make both of them "int" you get better code on the VAX.
	They do nothing if this is not Gosling redisplay, except for change the size
	of a structure that isn't used. A bit of a cheat.
*/
#define	XCHAR	char
#define	XSHORT	short

/*
	A video structure always holds an array of characters whose length is equal
	to the longest line possible. Only some of this is used if "ncol" isn't the
	same as "NCOL".
*/
typedef	struct	{
...
/*
	...
	redisplay, when the dynamic programming redisplay code is used. If no fancy
	redisplay, this isn't used. The trace index fields can be "char", and the
	score a "short", but this makes the code worse on the VAX.
 */
typedef	struct	{
...
/*
	...
	"setscores" routine, which is very compute intensive, to make the subscripts
	go away. It would be "SCORE	score[NROW][NROW]" in old speak. Look at
	"setscores" to understand what is up.
*/
...
void uline(int row, VIDEO *vvp, VIDEO *pvp);
...
/*
	...
	done at compile time. The original window is marked as needing full update,
	and the physical screen is marked as garbage, so all the right stuff happens
...
/*
	...
	host operating system. We erase the screen on exit.
*/
void
vttidy()
{
...
/*
	Move the virtual cursor to an origin 0 spot on the virtual display screen. I
	could store the column as a character pointer to the spot on the line, which
...
/*
	...
	whose width is not a multiple of 8, it is possible for the virtual cursor to
	hit the right margin before the next tab stop is reached. This makes the tab
	code loop if you are not careful. Three guesses how we found this.
*/
void
vtputc(int c)
{
	...
	if (vtcol >= ncol)
		...
		do {
			vtputc(' ');
		} while (vtcol<ncol && (vtcol&TABMASK)!=0);
...
/*
	...
	one in the "vscreen". The "pvp" is the one in the "pscreen". This is called
	to make the virtual and physical screens the same when display has done an
	update.
*/
void
...
/*
	Update a single line. This routine only uses basic functionality (no insert
	and delete character, but erase to end of line). The "vvp" points at the
	VIDEO structure for the line on the virtual screen, and the "pvp" is the
	same for the physical screen. Avoid erase to end of line when updating CMODE
	color lines, because of the way that reverse video works on most terminals.
*/
void
uline(int row, VIDEO *vvp, VIDEO *pvp)
{
	...
	if (vvp->v_color != pvp->v_color) {	// Wrong color, do a full redraw
		ttmove(row, 0);
		ttcolor(vvp->v_color);
		cp1 = &vvp->v_text[0];
		cp2 = &vvp->v_text[ncol];
...
/*
	...
	necessary. Also set the approximate redisplay cost. The validity of the hash
	code is marked by a flag bit. The cost understand the advantages of erase to
	end of line. Tuned for the VAX by Bob McNamara; better than it used to be on
...
/*
	...
	Older versions of this code implemented the score matrix by a two
	dimensional array of SCORE nodes. This put all kinds of multiply
	instructions in the code! This version is written to use a linear array and
	pointers, and contains no multiplication at all. The code has been carefully
	looked at on the VAX, with only marginal checking on other machines for
	efficiency. In fact, this has been tuned twice! Bob McNamara tuned it even
	more for the VAX, which is a big issue for him because of the 66 line X
	displays.
*/
void
...
/*
	...
	screen using an optimal sequence of redraws, insert lines, and delete lines.
	The "offs" is the origin 0 offset of the chunk of the screen we are about to
	update. The "i" and "j" are always started in the lower right corner of the
	matrix, and imply the size of the screen. A full screen traceback is called
	with offs=0 and i=j=nrow-1. There is some do-it-yourself double subscripting
	here, which is acceptable because this routine is much less compute
