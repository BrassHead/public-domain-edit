...
/*
	...
	redisplay code does). With no argument it defaults to 1. Because of the
	default, it works like in Gosling.
*/
int
...
/*
	Refresh the display. A call is made to the "ttresize" entry in the terminal
	handler, which tries to reset "nrow" and "ncol". They will, however, never
	be set outside of the NROW or NCOL range. If the display changed size,
	arrange that everything is redone, then call "update" to fix the display. We
	do this so the new size can be displayed. In the normal case the call to
	"update" in "main.c" refreshes the screen, and all of the windows need not
	be recomputed. Note that when you get to the "display unusable" message, the
	screen will be messed up. If you make the window bigger again, and send
	another command, everything will get fixed!
*/
int
...
/*
	...
	is only 1 window on the screen.
*/
int
...
/*
	...
	same; most of the work has to do with reframing the window, and picking a
	new dot. We share the code by having "move down" just be an interface to
	"move up".
*/
int
...
/*
	...
	command does not really move "."; it moves the frame).
*/
int
...
/*
	This command makes the current window the only window on the screen. Try to
	set the framing so that "." does not have to move on the display. Some care
	has to be taken to keep the values of dot and mark in the buffer structures
	right if the distruction of a window makes a buffer become undisplayed.
*/
int
...
/*
	Split the current window. A window smaller than 3 lines cannot be split. The
	only other error that is possible is a "malloc" failure allocating the
