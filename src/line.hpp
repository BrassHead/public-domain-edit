...
#define	KBLOCK	512			/* Kill buffer block size.	*/
...
/*
	...
	containing "used" characters. The block is always rounded up a bit. Return a
	pointer to the new block, or NULL if there isn't any memory left. Print a
...
/*
	...
	displayed in more than 1 window we change EDIT to HARD. Set MODE if the mode
...
/*
    ...
    greater than the place where you did the insert. Return TRUE if all is well,
    and FALSE on errors.
*/
int
linsert(int n, int c)
...
lreplace(int plen, char *st, int f)
{
	...
	/*
		Find the capitalization of the word that was found. f says use exact
		case of replacement string (same thing that happens with lowercase
		found), so bypass check.
	*/
	...
	/*
		...
		or scrunch out the excess). be careful with dot's offset.
	*/
	rlen = (int)strlen(st);
	doto = curwp->w_doto;
	if (plen > rlen)
	...
	/*
	 	do the replacement:  If was capital, then place first char as if upper,
	 	and subsequent chars as if lower.  If inserting upper, check replacement
	 	for case.
	*/
	while ((c = *st++&0xff) != '\0') {
