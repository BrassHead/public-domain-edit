...
/*
	Forward references
*/
void is_cpush(int cmd);
void is_lpush();
void is_pop();
int is_peek();
int is_undo(int *pptr, int *dir);
int is_find(int dir);
void is_prompt(int dir, int flag, int success);
void is_dspl(char *prompt, int flag);
...
/*
	If called with "dir" not one of SRCH_FORW or SRCH_BACK, this routine used to
	print an error message. It also used to return TRUE or FALSE, depending on
	if it liked the "dir". However, none of the callers looked at the status, so
	I just made the checking vanish.
*/
void
...
/*
	...
	string. The "flag" determines if a "[ ]" or ":" embelishment is used.
*/
void
...
queryrepl(int f, int n, int k)
{
	...
	int		kludge;			// Watch for saved line move
	...
	/*
	 	Search forward repeatedly, checking each time whether to insert or not.  
	 	The "!" case makes the check always true, so it gets put into a tighter
	 	loop for efficiency.

	 	If we change the line that is the remembered value of dot, then it is
	 	possible for the remembered value to move.  This causes great pain when
	 	trying to return to the non-existant line.
