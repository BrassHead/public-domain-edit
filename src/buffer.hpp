...
*/
void
itoa(char buf[], int width, int num)
{
...
/*
	...
	figures out the text, and puts it in the buffer whoses header is pointed to
	by the external "blistp". The "popblist" then pops the data onto the screen.
	Bound to "C-X C-B".
*/
int
...
/*
	...
	variable "blistp" onto the screen. This is used by the "listbuffers" routine
	(above) and by some other packages. Returns a status.
*/
int
popblist()
{
...
/*
	...
	buffers. Special buffers like the buffer list buffer don't count, as they
	are not in the list. Return FALSE if there are no changed buffers.
*/
int
anycb()
{
...
/*
	...
	a buffer and put it in the list of all buffers. Return pointer to the BUFFER
...
/*
	Create a buffer, by name. Return a pointer to the BUFFER header block, or
	NULL if the buffer cannot be created. The BUFFER is not put in the list of
	all buffers; this is called by "edinit" to create the buffer list buffer.
*/
BUFFER	*
